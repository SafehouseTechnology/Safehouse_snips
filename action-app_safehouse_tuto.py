...
#
If this skill is supposed to run on the satellite, #please get this mqtt connection info from < config.ini > #Hint: MQTT server is always running on the master device
MQTT_IP_ADDR = "localhost"
MQTT_PORT = 1883
MQTT_ADDR = "{}:{}".format(MQTT_IP_ADDR, str(MQTT_PORT))

class SafehouseTuto(object):
  ""
...
def __init__(self): #get the configuration
if needed
try:
...
self.start_blocking()
  ...
  # -- > Sub callback
function, one per intent
def askSafehouse_callback(self, hermes, intent_message): #terminate the session first
...
# action code goes here...
  print '[Received] intent: {}'.format(intent_message.intent.JohnG:askSafehouse)
...
function goes here...
  ...
  #-- > Master callback
function, triggered everytime an intent is recognized
def master_intent_callback(self, hermes, intent_message):
  coming_intent = intent_message.intent.JohnG:askSafehouse
if coming_intent == 'intent_1':
  self.askSafehouse_callback(hermes, intent_message)
